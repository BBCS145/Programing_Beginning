...
//8
static list* list_insert(node head)
{
	if (head == NULL)
	{
		cout << "先创建一个吧" << endl;
...
//9
static list* list_delete(node head)
{
	if (head == NULL)
	{
		cout << "先创建一个吧" << endl;
	...
	{
		int l, r;
		cout << "输入删除位置及数字(示例：1 2）" << endl;
		cin >> l >> r;
		if (l + 2 > get_length(head))
		{
			cout << "你想做甚" << endl;
			Sleep(2000);
			return head;
		...
		node temp = head;
		node temmie = (node)malloc(sizeof(node));
		for (int i = 1; i < l; i++)
		{
			temp = temp->next;
		}
		temmie = temp->next->next;
		delete temp->next;
		temp->next = temmie;
		return head;
	}
